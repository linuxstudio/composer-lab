...
/**
 * Voting transaction
 * @param {composer.sol.observer.voting.vote} vote
 * @transaction 
 */
async function vote(tx) {
    if (!tx.ifVotedAsset.votedOrNot) {
        ...
        let assetRegistry = await getAssetRegistry('composer.sol.observer.voting.candidateVote');
        await assetRegistry.update(tx.candidateVoteAsset);

        assetRegistry = await getAssetRegistry('composer.sol.observer.voting.ifVoted');
