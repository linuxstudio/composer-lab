...
/**
 * Voting transaction
 * @param {composer.sol.notification.voting.vote} vote
 * @transaction 
 */
async function vote(tx) {
    if (!tx.ifVotedAsset.votedOrNot) {
        ...
        let assetRegistry = await getAssetRegistry('composer.sol.notification.voting.candidateVote');
        await assetRegistry.update(tx.candidateVoteAsset);

        assetRegistry = await getAssetRegistry('composer.sol.notification.voting.ifVoted');
        tx.ifVotedAsset.votedOrNot = true;
        await assetRegistry.update(tx.ifVotedAsset);

        // Emit an event for the modified asset.
        let event = getFactory().newEvent('composer.sol.notification.voting', 'hasVoted');
