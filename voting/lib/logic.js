...
/**
 * Voting transaction
 * @param {composer.sample.voting.vote} vote
 * @transaction 
 */
async function vote(tx) {
    if (!tx.ifVotedAsset.votedOrNot) {
        ...
        let assetRegistry = await getAssetRegistry('composer.sample.voting.candidateVote');
        await assetRegistry.update(tx.candidateVoteAsset);

        assetRegistry = await getAssetRegistry('composer.sample.voting.ifVoted');
